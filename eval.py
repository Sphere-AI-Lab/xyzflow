...
from engine_xyzflow import train_one_epoch, evaluate



def get_args_parser():
    parser = argparse.ArgumentParser('XYZFlow inference', add_help=False)
    parser.add_argument('--batch_size', default=16, type=int,
                        help='Batch size per GPU (effective batch size is batch_size * # gpus')
    parser.add_argument('--epochs', default=400, type=int)

    # Model parameters
    parser.add_argument('--model', default='xyzflow_large', type=str, metavar='MODEL',
...
def main(args):
    ...
    # define the vae and xyzflow model
    vae = AutoencoderKL(embed_dim=args.vae_embed_dim, ch_mult=(1, 1, 2, 2, 4), ckpt_path=args.vae_path).cuda().eval()
    for param in vae.parameters():
        param.requires_grad = False

    model = xyzflow.__dict__[args.model](
    ...
    evaluate(model_without_ddp, vae, args, 0, batch_size=args.eval_bsz, log_writer=log_writer,
                cfg=args.cfg,flow_steps=args.num_steps)
    if device.type == "cuda":
        torch.cuda.synchronize()
    infer_elapsed = time.time() - infer_start
    if args.num_images:
        per_image = infer_elapsed / args.num_images
        print(f"Inference time: {infer_elapsed:.2f}s | per-image: {per_image:.4f}s")
    else:
