...
class XYZFlow(nn.Module):
    ...
    def sample_tokens(self, num_steps, cfg=1.0, label=None):
        ...
        for step in indices:
            ...
            z_sample = sampler.euler_sampler(self.sample_inference, latents, label, step,num_steps=num_steps, condition=prev_cond, cfg_scale=scaled_cfg, mask= self.mask[:,:,:sequence[step], :sequence[step]]).float()
            prev_cond = z_sample
